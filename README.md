...
- Full implementation of the **IBM Carbon Design System (g10)** with clean borders, flat palettes, and high-contrast styling.
- Bundled official typefaces: **Inter** for all UI controls and **Fira Code** for monospace code editors and logs.
- Dynamic responsive toolbar with crisp SVG vector icons.

### 📐 3D Geometry & Mesh Viewer (`Geometry` Tab)
- Powered by **PyVista** (`pyvistaqt.QtInteractor`) for high-performance 3D rendering of surface meshes (`.stl`, `.obj`).
...
- **Interactive Clipping & Slicing Planes**:
  - Real-time dynamic slicing along X, Y, or Z normals with position slider and normal inverter.
- **3D Point-to-Point Ruler & Measurement**:
  - Click-to-measure tool calculating exact Euclidean distance and coordinate deltas ($\Delta X, \Delta Y, \Delta Z$).
- **Rendering Controls**:
...
- Visual inspector and editor for all field files in the `0/` directory (`p`, `U`, `k`, `omega`, `epsilon`, `nut`, `alphat`, etc.).
...
2. **Parallel Execution** (`system/decomposeParDict`):
   - `numberOfSubdomains` with **1-click auto-detect CPU cores** button.
...
- Quick Find & Replace bar (Ctrl+F) with case-sensitivity and whole-word matching.
...
- Interactive live chart using `PySide6.QtCharts` plotting initial residuals per field vs. Time or Iteration.
...
- 1-click export of technical simulation reports via `ReportGenerator` (`QPdfWriter` + `QPainter`).
